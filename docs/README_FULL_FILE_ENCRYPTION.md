...
| `gui/app.py` | Vault tab has a "Files / Folders / Drives" sub-tab; Biometrics tab has a "File Attachments" sub-tab. Both use a shared `PathSelectionPanel` (multi-select files, folder picker, and an "Add Drive..." shortcut, all via `tkinter.filedialog`). |

File/folder/drive selection in the GUI is done entirely through
`tkinter.filedialog`'s standard picker dialogs. Native Windows drag-and-drop is
disabled in this release (see `gui/README.md` and `gui/windows_dnd.py`).
