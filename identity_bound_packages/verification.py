...
*** RESEARCH / DEMO ONLY ***
These are constant-time comparisons of SHA-256 hashes. They demonstrate the
...
and real credential storage needs a slow salted KDF (Argon2/scrypt).
"""
...
def verify_passphrase(identity, candidate: str) -> bool:
    return digests_equal(identity.passphrase_hash, hash_text(candidate))


def verify_voice(identity, candidate: str) -> bool:
    return digests_equal(identity.voice_hash, hash_text(candidate))
...
def verify_face(identity, candidate_template: str) -> bool:
    ...
    if identity.face_hash is None:
        return True
    return digests_equal(identity.face_hash, hash_text(candidate_template))
